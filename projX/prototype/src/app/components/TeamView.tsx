...
const initialTechnicians: Technician[] = mockTechnicianPerformance.map((tp, i) => ({
  id: tp.technicianId,
  name: tp.name,
  birthdate: '1985-06-15',
  gender: i % 2 === 0 ? 'Male' : 'Female',
  email: `${tp.name.toLowerCase().replace(' ', '.')}@smartsense.io`,
  phone: `+351 9${Math.floor(10000000 + Math.random() * 90000000)}`,
...
export const TeamView: React.FC = () => {
  ...
  const [archivedTechnicians, setArchivedTechnicians] = useState<Technician[]>([]);
  ...
  const avgRepairTime = technicians.length > 0
    ? technicians.reduce((s, t) => s + t.avgRepairTime, 0) / technicians.length
    : 0;
          ...
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 mt-2">
              ...
              <div>
                ...
                {newTech.skillSet.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {newTech.skillSet.map(skill => (
          ...
          <div className="space-y-4">
            {technicians.map(tech => (
              <div key={tech.id} className="border rounded-lg p-4">
                ...
                {/* Recent activity */}
                {mockBreakdowns.filter(b => b.technicianId === tech.id).length > 0 && (
                  <div className="mt-3 pt-3 border-t">
                    <div className="text-xs font-semibold text-gray-500 uppercase mb-2">Recent Activity</div>
                    <div className="space-y-2">
                      {mockBreakdowns.filter(b => b.technicianId === tech.id)
                        .sort((a, b) => b.date.getTime() - a.date.getTime())
                        .slice(0, 2)
                        .map(bd => (
                          <div key={bd.id} className="flex justify-between items-center text-xs bg-gray-50 p-2 rounded">
          ...
          {archivedTechnicians.length === 0 ? (
            ...
            <div className="space-y-3">
              {archivedTechnicians.map(tech => (
                <div key={tech.id} className="flex items-center justify-between p-4 border rounded-lg bg-gray-50">
                  <div className="flex items-center gap-3">
