...
export const Dashboard = () => {
  ...
  useEffect(() => {
    fetch(`${API_URL}/machines`, {
      ...
      .then((data) => {
        ...
        else if (data && Array.isArray(data.content)) setMachines(data.content);
        else setMachines([]);
      })
  ...
  return (
    <div className="space-y-6">
      ...
      {user?.role === "ADMIN" ? (
        // Admin-only simplified dashboard
        ...
        // Non-admin: keep existing dashboard (metrics + recent breakdowns + pending requests)
        <>
