...
const DirectorTeamView = () => {
  const [expandedId, setExpandedId] = useState(null);
  const totalRepairs = mockTechnicianPerformance.reduce((sum, tech) => sum + tech.completedRepairs, 0);
  const avgRepairTime = mockTechnicianPerformance.reduce((sum, tech) => sum + tech.avgRepairTime, 0) / mockTechnicianPerformance.length;
  return <div className="space-y-6">
    ...
    >
      ...
      /* Collapsible Recent Activity */
                  ...
                  {isExpanded && <div className="px-4 pb-4 pt-3 border-t">
                      <h4 className="font-semibold text-gray-700 mb-2 text-xs uppercase tracking-wider">Recent Activity</h4>
                      <div className="space-y-2">
                        {mockBreakdowns.filter((b) => b.technicianId === tech.technicianId).sort((a, b) => b.date.getTime() - a.date.getTime()).map((bd) => <div key={bd.id} className="flex justify-between items-start gap-4 text-sm bg-gray-50 p-3 rounded-md">
                        ...
                        {mockBreakdowns.filter((b) => b.technicianId === tech.technicianId).length === 0 && <p className="text-sm text-gray-500">No recent activity found.</p>}
                      </div>
                    </div>}
                </div>;
  })}
