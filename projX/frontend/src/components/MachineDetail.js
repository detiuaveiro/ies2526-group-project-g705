...
import { ArrowLeft, AlertCircle, Clock, ChevronDown, ChevronRight } from "lucide-react";
...
const MachineDetail = ({
  ...
  const [sensorData, setSensorData] = useState({ vibration: [], pressure: [], temperature: [] });
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [sensorsRes, problemsRes] = await Promise.all([
          fetch(`/api/v1/sensors/${machine.id}`),
          fetch(`/api/v1/problems/machine/${machine.id}`)
        ]);

        if (sensorsRes.ok) {
          const sensorsData = await sensorsRes.json();
          const groupedSensors = { vibration: [], pressure: [], temperature: [] };
          sensorsData.forEach(reading => {
            const point = { timestamp: reading.recordedAt, value: reading.value };
            if (reading.sensorType === "VIBRATION") groupedSensors.vibration.push(point);
            if (reading.sensorType === "PRESSURE") groupedSensors.pressure.push(point);
            if (reading.sensorType === "TEMPERATURE") groupedSensors.temperature.push(point);
          });
        ...
        if (problemsRes.ok) {
          ...
          const mappedBreakdowns = problemsData.map(p => {
            let repairTime = 0;
            if (p.resolved && p.startProblemDate && p.solvedProblemDate) {
              const start = new Date(p.startProblemDate);
              const end = new Date(p.solvedProblemDate);
              repairTime = Math.round((end - start) / (1000 * 60 * 60) * 10) / 10;
            }
            return {
              id: p.id,
              machineId: p.machine?.id || machine.id,
              title: `Problem #${p.id}`,
              date: p.detectedAt,
              resolved: p.resolved,
          ...
          setBreakdowns(mappedBreakdowns);
        }
      } catch (error) {
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between">
      ...
      {isTechnician && <div className="flex gap-2">
        {!isMaintenanceActive ? <Button
      ...
      {canRequestAssistance && !isTechnician && <Button variant="destructive" onClick={onRequestAssistance}>
        <AlertCircle className="w-4 h-4 mr-2" />
        Request Assistance
      </Button>}
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className={`text-2xl font-bold mt-1 capitalize ${machine.operationalStatus === "functional" ? "text-green-600" : machine.operationalStatus === "in-repair" ? "text-blue-600" : "text-gray-600"}`}>
            {machine.operationalStatus === "functional" ? "Functional" : machine.operationalStatus === "in-repair" ? "In Repair" : "Stopped"}
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={formatChartData(sensorData.temperature)}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="time" tick={{ fontSize: 12 }} />
              <YAxis label={{ value: "Value (\xB0C)", angle: -90, position: "insideLeft" }} />
              <Tooltip />
    ...
    {canViewHistory && <Card>
      ...
      <CardContent>
        {breakdowns.length === 0 ? <p className="text-gray-600 text-center py-4">No breakdown history available</p> : <div className="space-y-4">
          {breakdowns.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((breakdown) => <div
            key={breakdown.id}
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium flex items-center gap-2">
                  {expandedBreakdowns[breakdown.id] ? <ChevronDown className="w-4 h-4 text-gray-500" /> : <ChevronRight className="w-4 h-4 text-gray-500" />}
                  {breakdown.title}
                </div>
            ...
            {expandedBreakdowns[breakdown.id] && <div className="mt-4 pt-4 border-t space-y-3">
              ...
              {breakdown.resolved && <div className="flex gap-6 text-sm bg-white p-3 rounded-md border mt-2">
    ...
    {isMaintenanceActive && <EndMaintenanceDialog
