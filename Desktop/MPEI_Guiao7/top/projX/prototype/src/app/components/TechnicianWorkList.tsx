...
interface MaintenanceStatus {
  [key: string]: 'idle' | 'in-progress';
}

export const TechnicianWorkList: React.FC = () => {
  const [maintenanceStatus, setMaintenanceStatus] = useState<MaintenanceStatus>({});
  const [selectedMachine, setSelectedMachine] = useState<Machine | null>(null);
  const [assistanceDialogOpen, setAssistanceDialogOpen] = useState(false);

  // Ordenar máquinas por prioridade (prioridade mais baixa = mais urgente)
  const sortedMachines = [...mockMachines].sort((a, b) => {
    // Primeiro por status (breakdown > critical > warning > operational)
  ...
  // Identificar máquinas com hazard
  const hazardMachines = mockMachines.filter(m => 
    m.vibration > 80 || m.pressure > 110 || m.temperature > 90
  );

  const handleStartMaintenance = (machineId: string, machineName: string) => {
    setMaintenanceStatus(prev => ({ ...prev, [machineId]: 'in-progress' }));
    toast.success(`Manutenção iniciada para ${machineName}`);
  };

  const handleEndMaintenance = (machineId: string, machineName: string) => {
    setMaintenanceStatus(prev => ({ ...prev, [machineId]: 'idle' }));
    toast.success(`Manutenção concluída para ${machineName}`);
  };

  const handleRequestHelp = (machine: Machine) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'critical':
        return { bg: 'bg-orange-500', text: 'text-orange-700', border: 'border-orange-200', label: 'Crítico' };
      case 'breakdown':
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {isInMaintenance && (
                          <Badge className="bg-blue-600">Em Manutenção</Badge>
                        )}
                      </div>
